...
namespace Nekres.RotationTrainer.Core.Player.Controls {
    internal class ActionHint : ActionHintBase<Action> {
        ...
        protected override void Paint(SpriteBatch spriteBatch, Rectangle bounds) {
            ...
            this.Completed = _timer.Elapsed.TotalMilliseconds >= this.Action.Duration && _remReqActivations <= 0;

            if (this.Action.Duration > 0 && _timer.Elapsed.TotalMilliseconds < this.Action.Duration) {
                var duration = $"{(this.Action.Duration - _timer.Elapsed.TotalMilliseconds) / 1000:0.00}".Replace(',', '.');
            ...
            if (this.Action.Repetitions > 0 && _remReqActivations > 0) {
